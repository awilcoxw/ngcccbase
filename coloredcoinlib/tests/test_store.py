...
class TestStore(unittest.TestCase):
    ...
    def test_autocommit(self):
        ...
        self.assertTrue(not c.conn.isolation_level)
        os.remove(path)

    def test_table_exists(self):
        self.assertTrue(not self.store.table_exists('notthere'))

    def test_sync(self):
        self.assertTrue(not self.store.sync())

    def test_transaction(self):
        self.assertEqual(self.store.transaction(), self.dsc.conn)

    def test_colordata(self):
        self.assertTrue(not self.store.get(1, "1", 0))
        self.assertTrue(not self.store.get_any("1", 0))
        self.assertTrue(not self.store.get_all(1))
    ...
    def test_persistent(self):
        self.assertTrue(not self.persistent.get("tmp"))
    ...
    def test_meta(self):
        self.assertTrue(not self.meta.did_scan(1, "hash"))
        self.meta.set_as_scanned(1, "hash")
        self.assertTrue(self.meta.did_scan(1, "hash"))
        self.assertTrue(not self.meta.find_color_desc(1))
